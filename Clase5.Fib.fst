...
let rec fib_lin' (x:nat) : (nat & nat) =
  if x = 0 then (1, 1)
   else
      let (a, b) = fib_lin' (x - 1) in
      (b, a + b)
let fib_lin (n:nat) : nat = fst (fib_lin' n)
...
let rec fib_tail'_fib (n : nat) (a : nat) : Lemma (fib_tail' (fib a) (fib (a + 1)) n == fib (a + n)) =
  if n = 0 then ()
  else fib_tail'_fib (n - 1) (a + 1)
