...
let rec add (m n: nat) : Tot nat (decreases n) = if n = 0 then m else add (m + 1) (n - 1)



(* Demuestre la terminación de esta función *)

let rec countdown (x: int) : Tot nat (decreases x + 5) = if x <= - 5 then 0 else (countdown (x - 1))
...
let rec f (b: bool) (x: nat) : Tot nat (decreases %[b2nat b;x]) =
  if x = 0 then 0 else if b then 1 + f false (x + 100) else 1 + f b (x - 1)
...
let rec ff (x: nat) : Tot nat (decreases %[x;0]) = if x = 0 then 0 else gg (x - 1)
and gg (x: nat) : Tot nat (decreases %[x;1]) = 1 + ff x
...
let rec sum (xs: list int) : int =
  ...
  (* Porque sum se llama con el tail de la list, que es un componente de su constructor Cons y por lo tanto cumple que tl xs << xs *)
  if Nil? xs then 0 else List.Tot.hd xs + sum (List.Tot.tl xs)
...
let rec sum' (xs: list int) : Tot int =
  ...
  (* Porque no puede probar que init xs << xs *)
  if Nil? xs then 0 else last xs + sum' (init xs)
...
let rec ack' (n m: nat) : Tot nat (decreases %[m;n]) =
  if m = 0 then n + 1 else if n = 0 then ack' 1 (m - 1) else ack' (ack' (n - 1) m) (m - 1)