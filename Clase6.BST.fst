...
let rec insert0_bst (x:int) (t:bst0) : Lemma (requires is_bst t) (ensures is_bst (insert0 x t)) =
  ...
  | N (l, z, r) ->
    if x = z then ()
    else if x < z then (insert0_all_lt x l z; insert0_bst x l)
    else (insert0_all_gt x r z; insert0_bst x r)
...
let rec all_lt_notin (y:int) (x:int) (t:bst) : Lemma (requires all_lt y t /\ x >= y) (ensures not (in_tree x t)) =
  match t with
  | L -> ()
  | N (l, z, r) ->
    all_lt_notin y x l;
...
let rec all_gt_notin (y:int) (x:int) (t:bst) : Lemma (requires all_gt y t /\ x <= y) (ensures not (in_tree x t)) =
  match t with
  | L -> ()
  | N (l, z, r) ->
    all_gt_notin y x l;
...
let rec member_ok (x:int) (t:bst) : Lemma (member x t == in_tree x t) =
  ...
  | N (l, y, r) ->
    member_ok x l;
    member_ok x r;
    if x > y then all_lt_notin y x l
    else if x < y then all_gt_notin y x r
    else ()
...
=
  match t with
  | N (L, x, r) -> ()
  | N (l, x, r) -> extract_min_es_bst l; extract_min_preserva_all_lt x l
