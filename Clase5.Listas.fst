...
= match xs with
  | [] -> ()
  | x::xs' ->
    assert((rev_int (xs @ ys)) = (snoc (rev_int (xs' @ ys)) x));
    rev_append_int xs' ys;
    assert((rev_int (xs @ ys)) = (snoc (rev_int ys @ rev_int xs') x));
    snoc_append (rev_int ys) (rev_int xs') x;
    assert((snoc (rev_int ys @ rev_int xs') x) = ((rev_int ys) @ (snoc (rev_int xs') x)));
    ()

let rec snoc_ok (xs : list int) (x : int)
...
= match xs with
  | [] -> ()
  | x::xs' ->
    snoc_ok (rev_int xs') x;
    assert(rev_int (rev_int (x::xs')) = rev_int ((rev_int xs') @ [x]));
